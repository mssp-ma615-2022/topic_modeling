...
New_topicNames <- c('Villian','Horror','Comedy','Oscar','Family','Arts','History','Sitcom','Crime','kids','Sciences','Dialogue','Musicals','Action','Female')
names(countsOfPrimaryTopics) <- New_topicNames
...
ggplot(counts) +
  ...
  labs(x = "film types", y = "counts") +
  theme_minimal()

```

```{r}
# assign each document of topic with the highest probability
imdb_classed <- imdb
for (i in 1:nDocs(review_dtm)) {
  max_index <- which.max(theta[i, ])[[1]] # select topic distribution for document i
  # get first element position from ordered list
  imdb_classed$category[i] <- topicNames[max_index]
